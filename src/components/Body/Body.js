...
export default function Body({ topMovies, nowMovies, upMovies }) {
  ...
  useEffect(() => {
    ...
    const fetchMovies = async (url, setShowMovies, loadingKey) => {
      try {
    ...
    const urls = [
      {
        url: `https://api.themoviedb.org/3/movie/top_rated?api_key=7a47242793d59eb1570389827de8affd&language=en-US&page=${n}`,
        setShowMovies: setShowMovies1,
        loadingKey: "top",
      },
      {
        url: `https://api.themoviedb.org/3/movie/now_playing?api_key=7a47242793d59eb1570389827de8affd&language=en-US&page=${n}`,
        setShowMovies: setShowMovies2,
        loadingKey: "now",
      },
      {
        url: `https://api.themoviedb.org/3/movie/popular?api_key=7a47242793d59eb1570389827de8affd&language=en-US&page=${n}`,
        setShowMovies: setShowMovies3,
        loadingKey: "popular",
      },
    ];

    urls.forEach(({ url, setShowMovies, loadingKey }) =>
      fetchMovies(url, setShowMovies, loadingKey)
    );
  }, [n, categoryNumber]);
  ...
  return (
    <>
      <div
        className="w-4/5 mx-auto my-4 flex flex-row h-[40px] lg:space-x-16 
      md:space-x-10 sm:space-x-0"
      >
          ...
          return (
            <>
              ...
              >
                ...
                <p
                  className="text-textt xl:text-3xl my-auto ml-2 sm:text-lg 
                md:text-2xl"
                >
                  {e}
                </p>
      ...
      {categoryNumber === 1 && (
        ...
        >
          {loading.top ? (
            ...
            <div className="skeleton min-w-[45%]"></div>
          )}
          {loading.top ? (
            <div className="min-w-[45%]">
      ...
      {categoryNumber === 2 && (
        <div
          className="grid lg:grid-cols-2 md:grid-cols-1 lg:gap-x-[10%] mx-[10%]
        sm:grid-cols-1 md:gap-4 sm:gap-4"
        >
          {loading.now ? (
            <div className="min-w-[45%]">
